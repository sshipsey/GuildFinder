...
def fetchpage1():
    return requests.get(baseURL + '?faction=' + faction).content
...
def CreateSoup(v):
    return zip_longest((tag.nobr.string for tag in BeautifulSoup(v, parser).find_all("a", {"class":"guild " + faction})), (tag.b.string for tag in BeautifulSoup(v, parser).find_all("span", {"class":"innerLink ratingProgress"})))
